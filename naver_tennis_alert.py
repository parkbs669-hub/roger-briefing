...
def build_html_email(posts: list[dict]) -> str:
    ...
    def render_cards(items: list[dict], color: str) -> str:
        html = ""
        for p in items:
            author = f'<span style="color:#6b7280;font-size:12px;">✍️ {p["author"]}</span>' if p["author"] else ""
            cafe   = f'<span style="color:#6b7280;font-size:12px;">☕ {p["cafe_name"]}</span>' if p["cafe_name"] else ""
            ...
            <div style="padding:14px;background:#f8fafc;border-radius:8px;
              ...
              <div style="margin:5px 0 4px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                {author}{cafe}
                <span style="color:#9ca3af;font-size:11px;">{fmt_date(p['date'])}</span>
                <span style="background:{color}22;color:{color};font-size:11px;
                             padding:1px 8px;border-radius:10px;">{p['keyword']}</span>
              </div>
              <div style="font-size:13px;color:#475569;line-height:1.5;">{desc}</div>
              <a href="{p['link']}"
                 style="display:inline-block;margin-top:6px;font-size:12px;
                        color:{color};text-decoration:none;">자세히 보기 →</a>
            </div>"""
        return html
    ...
    def render_section(items: list[dict], icon: str, label: str, color: str) -> str:
        ...
        <div style="background:#fff;border-radius:12px;padding:20px;margin-bottom:16px;">
          <h2 style="margin:0 0 14px;font-size:15px;color:#374151;
                     border-bottom:2px solid #e5e7eb;padding-bottom:8px;">
            {icon} 네이버 {label} ({len(items)}건)
          </h2>
          {render_cards(items, color)}
        </div>"""
...
<div style="max-width:660px;margin:0 auto;padding:24px 16px;">
  ...
  {render_section(blogs, "📝", "블로그", "#059669")}
  {render_section(cafes, "☕", "카페",   "#f59e0b")}

  <div style="background:#fff;border-radius:12px;padding:14px;margin-bottom:16px;">
